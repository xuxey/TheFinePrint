<script lang="ts">
	...
	import { ACCESS_CODE, API_URL, POLLING_INTERVAL_MS } from '../constants';
	...
	const sendImageRequest = async () => {
		...
		const response = await fetch(
			`${API_URL}/summarise_image?${new URLSearchParams({ url, access_code: ACCESS_CODE })}`,
			{
				method: 'POST',
				body: data
			}
...
<div class="flex flex-col gap-8 w-full items-center">
	...
	<button
		on:click|preventDefault={sendImageRequest}
	...
	>
		...
		<span class="text-white font-semibold font-sans">Analyze this Image</span>
	</button>
